...
```{r tab-BigCity, tab.cap="Total de personas, ingresos y gastos"}
  head(FrameI, 8) %>%
  tabla_fmt()
```
...
```{r tab-sizes, tab.cap="Tamaños de muestra y factores de expansión por estrato"}
head(sizes, 8) %>%
  tabla_fmt()
```
...
```{r tab-frameII, tab.cap="Muestra de UPMs con información del marco de muestreo (primeras 8 filas)"}
head(FrameII, 8) %>%
  select(Stratum:Zone) %>%
  tabla_fmt()
```
...
```{r tab-encuesta, tab.cap="Primeras observaciones de la muestra final seleccionada"}
head(encuesta, 8) %>%
  select(Stratum:Zone) %>%
  tabla_fmt()
```
...
```{r tab-cal-totales, tab.cap="Población representada por zona y sexo tras la calibración"}
