...
sub_Hombre %>%
    ...
    summarise(prop = survey_prop(vartype = c("se","ci")))
```
```{r tab-prop-zona-empleo-hombre, tab.cap="Proporción de hombres por zona y estado de ocupación", echo=FALSE}
sub_Hombre %>%
    filter(!is.na(Employment)) %>%
    group_by(Zone, Employment) %>% 
    summarise(prop = survey_prop(vartype = c("se","ci"))) %>%
...
sub_Mujer %>% 
    ...
    summarise(prop = survey_prop(vartype = c("se","ci")))
```
```{r tab-prop-zona-empleo-mujer, tab.cap="Proporción de mujeres por zona y estado de ocupación", echo=FALSE}
sub_Mujer %>% 
    filter(!is.na(Employment)) %>%
    group_by(Zone, Employment) %>% 
    summarise(prop = survey_prop(vartype = c("se","ci"))) %>%
...
diseno %>% 
  ...
  summarise(Prop = survey_prop(vartype =  c("se", "ci")))
```
```{r tab-prop-edad-pobreza, echo=TRUE, tab.cap="Proporción por grupo de edad y condición de pobreza", echo=FALSE}
diseno %>% 
  group_by(edad_18, pobreza) %>% 
  summarise(Prop = survey_prop(vartype =  c("se", "ci"))) %>%
  tabla_fmt()
...
diseno %>% 
  ...
  summarise(Prop = survey_prop(vartype =  c("se", "ci")))
```
```{r tab-prop-edad-desempleo, echo=TRUE, tab.cap="Proporción por grupo de edad y condición de desempleo", echo=FALSE}
diseno %>% 
  filter(!is.na(desempleo)) %>%
  group_by(edad_18, desempleo) %>% 
  summarise(Prop = survey_prop(vartype =  c("se", "ci"))) %>%
...
sub_Rural %>% 
  ...
  summarise(Prop = survey_prop(vartype =  c("se", "ci")))
```
```{r tab-prop-edad-rural, tab.cap="Proporción por grupo de edad en zona rural", echo=FALSE}
sub_Rural %>% 
  group_by(edad_18) %>% 
  summarise(Prop = survey_prop(vartype =  c("se", "ci"))) %>%
  tabla_fmt()
...
tab_sex_desempleo
```
```{r tab-svyby-desempleo-sexo, tab.cap="Proporción en condición de desempleo por sexo", echo=FALSE}
tab_sex_desempleo <- svyby(formula = ~desempleo, by = ~Sex,
                           design  = diseno %>% filter(!is.na(desempleo)),
                           FUN     = svymean, na.rm=T, covmat = TRUE,
                           vartype = c("se", "ci"))
...
vcov(tab_sex_desempleo) %>%
  data.frame()
```
```{r tab-vcov-sex-desempleo, tab.cap="Matriz de varianzas y covarianzas de la proporción de desempleo por sexo", echo=FALSE}
vcov(tab_sex_desempleo) %>%
  data.frame() %>%
  tabla_fmt(digits = 10)
```
...
tab_region_pobreza
```
```{r tab-svyby-pobreza-region, tab.cap="Proporción en condición de pobreza por región", echo=FALSE}
tab_region_pobreza <- svyby(formula = ~pobreza, by = ~Region,
                            design = diseno %>% filter(!is.na(desempleo)),
                            FUN = svymean, na.rm=T, covmat = TRUE,
                            vartype = c("se", "ci"))
