...
```{r tab-tamano-zona, eval=TRUE, tab.cap="Tamaño poblacional estimado por zona geográfica"}
diseno %>% 
     group_by(Zone) %>% 
...
De manera análoga, es posible estimar el número de personas en cada nivel de pobreza:
  
```{r tab-tamano-pobreza, eval=TRUE, tab.cap="Tamaño poblacional estimado por condición de pobreza"}
diseno %>% 
  group_by(Poverty) %>% 
...
Otra variable categórica relevante en encuestas de hogares es el estado de ocupación. A continuación se presenta el código correspondiente:
  
```{r tab-tamano-empleo, eval=TRUE, tab.cap="Tamaño poblacional estimado por estado de ocupación"}
diseno %>% 
...
Finalmente, utilizando `group_by` es posible desagregar las estimaciones en niveles más detallados. Por ejemplo, estado ocupacional por nivel de pobreza:
  
```{r tab-tamano-empleo-pobreza, echo=TRUE, eval=TRUE, tab.cap="Tamaño poblacional estimado por estado de ocupación y condición de pobreza"}
diseno %>% 
  filter(!is.na(Employment)) %>%
...
A continuación se ilustra cómo obtener la proporción de personas por zona usando el diseño muestral previamente definido:

```{r tab-prop-zona, eval=TRUE, tab.cap="Proporción de personas por zona geográfica"}
diseno %>% 
  group_by(Zone) %>% 
...
La librería `survey` también cuenta con la función `survey_prop()`, diseñada específicamente para estimar proporciones, generando resultados equivalentes, así:

```{r tab-prop-zona2, eval=TRUE, tab.cap="Proporción de personas por zona geográfica (función survey_prop)"}
diseno %>% 
  group_by(Zone) %>% 
...
```{r tab-prop-sexo-urbano, eval=TRUE, tab.cap="Proporción de hombres y mujeres en zona urbana"}
sub_Urbano %>% 
  group_by(Sex) %>% 
  summarise(prop = survey_prop(vartype = c("se","ci"))) %>%
  gt() %>% cols_align("center")
...
De manera análoga se puede realizar el mismo ejercicio, pero ahora con la zona rural.


```{r tab-prop-sexo-rural, eval=TRUE, tab.cap="Proporción de hombres y mujeres en zona rural"}
sub_Rural %>% 
  group_by(Sex) %>% 
  summarise(n    = unweighted(n()),
...
```{r tab-prop-zona-hombre, eval=TRUE, tab.cap="Distribución por zona en la subpoblación masculina"}
sub_Hombre %>% 
  group_by(Zone) %>% 
  summarise(prop = survey_prop(vartype = c("se","ci"))) %>%
  gt() %>% cols_align("center")
...
```{r tab-prop-zona-pobreza-hombre, eval=TRUE, tab.cap="Proporción de hombres por zona y condición de pobreza"}
sub_Hombre %>%
...
Otro ejemplo podría ser estimar la proporción de hombres empleados o no por zona, que se realiza de la siguiente manera:

```{r tab-prop-zona-empleo-hombre, eval=TRUE, tab.cap="Proporción de hombres por zona y estado de ocupación"}
sub_Hombre %>%
    filter(!is.na(Employment)) %>%
...
De manera análoga se hace este mismo ejercicio para las mujeres:

```{r tab-prop-zona-empleo-mujer, eval=TRUE, tab.cap="Proporción de mujeres por zona y estado de ocupación"}
sub_Mujer %>% 
    filter(!is.na(Employment)) %>%
    group_by(Zone, Employment) %>% 
    summarise(prop = survey_prop(vartype = c("se","ci"))) %>%
...
Otro parámetro que es de interés es estimar en encuestas de hogares la cantidad de personas menores y mayores de edad en los hogares. A continuación, se realiza un estimación de menores y mayores a 18 años cruzado por pobreza:

```{r tab-prop-edad-pobreza, echo=TRUE, eval=TRUE, tab.cap="Proporción por grupo de edad y condición de pobreza"}
diseno %>% 
...
```{r tab-prop-edad-desempleo, echo=TRUE, eval=TRUE, tab.cap="Proporción por grupo de edad y condición de desempleo"}
diseno %>% 
...
Como se observa, el 2% de los menores de edad y el 4% de los mayores de edad están desempleados. Por otro lado, si el objetivo ahora es estimar la proporción de menores de edad en la zona rural se realiza de la siguiente manera:
...
Como se mencionó al inicio del capítulo, es posible categorizar una variable de tipo cuantitativo como por ejemplo la edad y cruzarla con la variable que categoriza la empleabilidad. A continuación, se clasifica la edad de las mujeres en rangos de 18 a 35 años, y otra edad.


```{r tab-prop-edad-rango-empleo-mujer, eval=TRUE, tab.cap="Proporción de mujeres por rango de edad y estado de ocupación"}
sub_Mujer %>% 
  filter(!is.na(Employment)) %>% 
...
```{r tab-contingencia-pobreza-sexo, eval=TRUE, tab.cap="Proporción de hombres y mujeres en condición de pobreza y no pobreza"}
dsg <- diseno %>% 
...
```{r tab-svyby-pobreza-sexo, eval=TRUE, tab.cap="Proporción de hombres y mujeres por condición de pobreza (svyby)"}
tab_Sex_Pobr <- svyby(formula = ~Sex, by =  ~pobreza, design = dsg, FUN = svymean) 

tab_Sex_Pobr %>% gt() %>% cols_align("center")
```
...
Para la estimación de los intervalos de confianza se utiliza la función `confint`:
...
```{r tab-svyby-empleo-sexo, echo=TRUE, eval=TRUE, tab.cap="Proporción de hombres y mujeres por estado de ocupación (svyby)"}
tab_Sex_Ocupa <- svyby(formula = ~Sex,  by = ~Employment,
                       design = diseno, FUN = svymean)
tab_Sex_Ocupa %>% gt() %>% cols_align("center")
```
...
```{r tab-svyby-region-pobreza, eval=TRUE, tab.cap="Proporción en condición de pobreza por región"}
tab_region_pobreza <- svyby(formula = ~as.factor(pobreza),  by = ~Region, 
                            design =  diseno, FUN = svymean)
tab_region_pobreza %>% gt() %>% cols_align("center")
```
...
```{r tab-interaccion-sexo-pobreza, echo=TRUE, eval=TRUE, tab.cap="Proporciones conjuntas de sexo y condición de pobreza"}
tab_Sex_Pobr <- svymean(x = ~interaction (Sex, pobreza), design = diseno, 
                        se=T, na.rm=T, ci=T, keep.vars=T)

tab_Sex_Pobr %>% as.data.frame() %>% gt() %>% cols_align("center")
...
tab_sex_pobreza %>% gt() %>% fmt_number(columns = where(is.numeric), decimals = 4) %>% cols_align("center")
```
...
tab_sex_desempleo <- svyby(formula = ~desempleo, by = ~Sex, 
                           ...
                           vartype = c("se", "ci"))
tab_sex_desempleo %>% gt() %>% fmt_number(columns = where(is.numeric), decimals = 4) %>% cols_align("center")
```
...
tab_region_desempleo <- svyby(formula =  ~desempleo, by = ~Region, 
                              ...
                              FUN     = svymean, na.rm=T, covmat = TRUE,
                              vartype = c("se", "ci"))
tab_region_desempleo %>% gt() %>% fmt_number(columns = where(is.numeric), decimals = 4) %>% cols_align("center")
...
```{r tab-vcov-region-desempleo, echo=TRUE, eval=FALSE, tab.cap="Matriz de varianzas y covarianzas del desempleo por región"}
...
tab_region_pobreza <- svyby(formula = ~pobreza, by = ~Region, 
                            ...
                            FUN = svymean, na.rm=T, covmat = TRUE,
                            vartype = c("se", "ci"))
tab_region_pobreza %>% gt() %>% fmt_number(columns = where(is.numeric), decimals = 4) %>% cols_align("center")
```
...
La construcción de un gráfico de barras en `R` requiere, en primer lugar, calcular las estimaciones que se desean representar. Para ello se utilizan las funciones `survey_total()` o `survey_prop()` dentro del paquete `srvyr`. A continuación se ilustra con el tamaño poblacional por zona:

```{r fig-barras-zona, fig.cap="Gráfico de barras del tamaño poblacional por zona geográfica", fig.align='center', out.width='70%', echo=TRUE, eval=TRUE}
tamano_zona <- diseno %>%
  group_by(Zone) %>%
  summarise(Nd = survey_total(vartype = c("se", "ci")))
...
El gráfico anterior puede extenderse a variables con más categorías. Por ejemplo, para la variable de pobreza:

```{r fig-barras-pobreza, fig.cap="Gráfico de barras del tamaño poblacional por condición de pobreza", fig.align='center', out.width='70%', echo=TRUE, eval=TRUE}
...
Una de las ventajas de los gráficos de barras es que pueden extenderse fácilmente a comparaciones entre dos variables categóricas. Por ejemplo, se puede analizar el tamaño poblacional por nivel de pobreza cruzado con el estado de ocupación:

```{r fig-barras-empleo-pobreza, fig.cap="Gráfico de barras del tamaño poblacional por estado de ocupación y pobreza", fig.align='center', out.width='80%', echo=TRUE, eval=TRUE}
...
También es posible presentar proporciones en lugar de conteos. Por ejemplo, la proporción de hombres en condición de pobreza por zona:

```{r fig-barras-prop-zona-hombre, fig.cap="Proporción de hombres en condición de pobreza por zona geográfica", fig.align='center', out.width='70%', echo=TRUE, eval=TRUE}
...
De forma análoga, se puede graficar la proporción de hombres en condición de pobreza desagregada por región:

```{r fig-barras-prop-region-hombre, fig.cap="Proporción de hombres en condición de pobreza por región", fig.align='center', out.width='75%', echo=TRUE, eval=TRUE}
