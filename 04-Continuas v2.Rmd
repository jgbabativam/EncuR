...
```{r tab-total-ingreso, eval=TRUE, tab.cap="Estimación del total de ingresos de los hogares con intervalo de confianza al 95%"}
diseno %>% 
...
```{r tab-total-gasto, tab.cap="Estimación del total de gastos de los hogares con intervalo de confianza al 90%"}
diseno %>%
...
```{r tab-total-ingreso-sexo, tab.cap="Total de ingresos de los hogares desagregado por sexo"}
diseno %>% 
  group_by(Sex) %>%
...
```{r tab-media-ingreso, tab.cap="Estimación de la media de ingresos de los hogares"}
diseno %>%
...
```{r tab-media-gasto, tab.cap="Estimación de la media de gastos de los hogares"}
diseno %>%
...
```{r tab-media-gasto-sexo, tab.cap="Media de gastos de los hogares desagregada por sexo"}
diseno %>% 
  group_by(Sex) %>%
...
```{r tab-media-gasto-zona, tab.cap="Media de gastos de los hogares desagregada por zona"}
diseno %>% 
  group_by(Zone) %>%
  cascade(Media = survey_mean(Expenditure, 
                              level = 0.95,
...
```{r tab-media-gasto-zona-sexo, tab.cap="Media de gastos de los hogares desagregada por zona y sexo"}
diseno %>% 
  group_by(Zone, Sex) %>%
  cascade(Media = survey_mean(Expenditure, 
                              level = 0.95,
...
```{r tab-sd-ingreso-zona, tab.cap="Desviación estándar de los ingresos desagregada por zona"}
diseno %>% 
  group_by(Zone) %>%
...
```{r tab-sd-ingreso-zona-sexo, tab.cap="Desviación estándar de los ingresos desagregada por zona y sexo"}
diseno %>% 
  group_by(Zone, Sex) %>%
  summarise(Sd = sqrt(survey_var(Income,
                                 level = 0.95,
...
```{r tab-mediana-gasto, tab.cap="Estimación de la mediana de los gastos de los hogares"}
diseno %>% 
...
```{r tab-mediana-gasto-zona, tab.cap="Estimación de la mediana de los gastos desagregada por zona"}
diseno %>% 
  group_by(Zone) %>%
  summarise(mediana = survey_median(Expenditure,
                                    level = 0.95,
...
```{r tab-mediana-gasto-sexo, tab.cap="Estimación de la mediana de los gastos desagregada por sexo"}
diseno %>% 
  group_by(Sex) %>%
  summarise(mediana = survey_median(Expenditure,
                                    level = 0.95,
...
```{r tab-cuantil-gasto, tab.cap="Estimación del primer cuartil de los gastos de los hogares"}
diseno %>%
...
```{r tab-cuantil-gasto-zona-sexo, tab.cap="Primer cuartil de los gastos desagregado por zona y sexo"}
diseno %>% 
  group_by(Zone, Sex) %>%
  summarise(q = survey_quantile(Expenditure,
                                quantiles = 0.25,
...
```{r tab-razon-gasto-ingreso, eval=TRUE, tab.cap="Razón gasto/ingreso de los hogares"}
diseno %>% 
...
```{r tab-razon-sexo, tab.cap="Razón de mujeres por hombre en la población"}
diseno %>% 
...
```{r tab-razon-sexo-rural, tab.cap="Razón de mujeres por hombre en zona rural"}
...
```{r tab-razon-gasto-ingreso-mujer, tab.cap="Razón gasto/ingreso en hogares con jefatura femenina"}
sub_Mujer %>% 
  summarise(Razon = survey_ratio(numerator = Expenditure,
                                 denominator = Income,
                                 level = 0.95,
...
```{r tab-razon-gasto-ingreso-hombre, tab.cap="Razón gasto/ingreso en hogares con jefatura masculina"}
sub_Hombre %>% 
  summarise(Razon = survey_ratio(numerator = Expenditure,
                                 denominator = Income,
                                 level = 0.95,
...
```{r tab-gini, eval=TRUE, tab.cap="Estimación del coeficiente de Gini para el ingreso de los hogares"}
library(convey)
...
A continuación se presenta un histograma de la variable `Income` incorporando los pesos muestrales (`wk`):

```{r fig-hist-ingreso-ponde, fig.cap="Histograma ponderado del ingreso de los hogares", fig.align='center', out.width='65%', echo=TRUE, eval=TRUE}
plot1_Ponde <- ggplot(
  data = encuesta,
...
Para efectos de contraste, se incluye también el histograma sin ponderar y el de la población completa (`BigCity`):

```{r fig-hist-comparacion, fig.cap="Comparación de histogramas del ingreso: poblacional, ponderado y sin ponderar", fig.align='center', out.width='90%', echo=TRUE, eval=TRUE}
...
Como se puede observar, el histograma que mejor se aproxima al poblacional es el que incorpora los pesos de muestreo. Los histogramas permiten además **comparar subgrupos** mediante el argumento `fill`. Por ejemplo, para comparar zonas:

```{r fig-hist-zona, fig.cap="Histograma ponderado del ingreso por zona geográfica", fig.align='center', out.width='65%', echo=TRUE, eval=TRUE}
...
Las curvas de densidad suavizadas pueden superponerse a los histogramas con `geom_density`, aportando una visión continua de la distribución:
...
El boxplot —introducido por Tukey (1977)— resume gráficamente la distribución de una variable a través de sus cuartiles e identifica valores atípicos. Para graficar boxplots ponderados con `ggplot2` se utiliza `geom_boxplot`:

```{r fig-boxplot-zona-sexo, fig.cap="Boxplots ponderados del ingreso por zona y sexo", fig.align='center', out.width='90%', echo=TRUE, eval=TRUE}
...
Cuando se requiere mayor precisión en la estimación, el paquete `survey` ofrece las funciones `svyhist()` y `svyboxplot()`, que incorporan directamente el diseño muestral complejo:

```{r fig-svyhist-svybox, fig.cap="Histograma y boxplot del ingreso ajustados al diseño muestral complejo", fig.align='center', out.width='80%', eval=TRUE}
par(mfrow = c(1, 2))
...
Para tamaños muestrales moderados, el tamaño del símbolo puede representar el peso de cada observación:

```{r fig-scatter-basico, fig.cap="Diagrama de dispersión entre ingreso y gasto de los hogares", fig.align='center', out.width='65%', echo=TRUE, eval=TRUE}
plot20_Ponde <- ggplot(
  data = encuesta,
...
Para agregar una variable de agrupamiento se introducen los argumentos `shape` y `color`:

```{r fig-scatter-zona, fig.cap="Diagrama de dispersión entre ingreso y gasto por zona geográfica", fig.align='center', out.width='65%', echo=TRUE, eval=TRUE}
plot22_Ponde <- ggplot(
  data = encuesta,
