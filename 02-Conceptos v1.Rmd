...
Para el ejemplo se emplean los datos *BigCity*, descritos en la sección \@ref(sec-motivacion). Una vez cargadas las librerías, se agrupan las observaciones por unidad primaria de muestreo (UPM) y estrato, obteniendo los totales de personas, ingresos y gastos.
...
```{r tab.BigCity, tab.cap="Total de personas, ingresos y gastos", results='asis'}
  head(FrameI, 8) %>% gt()
```

Paso seguido, se calculan los tamaños poblacionales por estrato ($N_{Ih}$) y sus respectivos tamaños de muestra ($n_{Ih}$).
...
Una vez se tiene la base de datos con la muestra de UPMs, se procede a seleccionar aquellas variables que son de interés para el estudio.

```{r}
head(FrameII, 8) %>% 
  select(Stratum:Zone) %>% gt()
```
...
```{r}
head(encuesta, 8) %>% select(Stratum:Zone) %>% gt()
```
...
Después de la calibración, los pesos ajustados se asignan a la encuesta y reproducen exactamente los totales poblacionales establecidos en el vector totales. Esto puede verificarse sumando los pesos calibrados:
