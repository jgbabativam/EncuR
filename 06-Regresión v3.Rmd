...
Ahora bien, para observar los parámetros poblacionales del modelo se utilizará la función `modelsummary` de la librería `modelsummary` de la siguiente manera, presentada en la Tabla \@ref(tab:tab-modelo-poblacional):
...
Ahora, haciendo un Scatterplot con los datos encuesta pero utilizando los factores de expansión del diseño se debe agregar ` mapping = aes(weight = wk)` en la función `geom_smooth`como sigue, presentada en la Figura \@ref(fig:fig-scatter-muestra-ponde):
...
Por último y a modo de resumen se muestra un gráfico donde se encuentran depositados todos los modelos estimados anteriormente y así poder comparar de manera gráfica su ajuste, presentada en la Figura \@ref(fig:fig-comparacion-modelos):
...
```{r tab-r2adj, echo=FALSE, tab.cap="Estimación del coeficiente de determinación ajustado"}
data.frame(
...
La Tabla \@ref(tab:tab-r2adj) muestra que el $R_{adj}^{2}$ resulta levemente inferior al $R^{2}$, como es esperable dado el ajuste por el número de parámetros. Su interpretación depende del contexto disciplinar, tal como se discutió al inicio de esta sección.
...
```{r fig.align='center', out.width='65%', echo=TRUE, eval=FALSE}
...
```{r prep-residuales-covariables, echo=TRUE, eval=TRUE}
library(patchwork)
...
```{r fig.align='center', out.width='80%', echo=TRUE, eval=FALSE}
...
```{r fig.align='center', out.width='65%', eval=FALSE}
library(svydiags)
d_cook = data.frame(
...
Ahora bien, se desea observar si hay observaciones influyentes pero utilizando $D_{f}Betas_{\left(i\right)j}$ se realiza con la función `svydfbetas` como se muestra a continuación, presentada en la Tabla \@ref(tab:tab-dfbetas-primeros):
...
```{r fig.align='center', out.width='80%', eval=FALSE, echo=TRUE}
...
A continuación, se presenta cómo se realiza la estimación del valor esperado, primero se estiman los parámetros del modelo, presentada en la Tabla \@ref(tab:tab-coeficientes-modelo):
...
$$
Para calcular la varianza de la estimación, primero se deben obtener las varianzas de la estimación de los parámetros, presentada en la Tabla \@ref(tab:tab-vcov-modelo):
